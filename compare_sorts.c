...
//if instead of setting slot to arr size you set slot to min(arrSize, vs max/min val in arr) worst case O(n) <--- bucket sort if small amount of buckets
...
int testAddGet(int* table,stype* arr){
   int slot;
   int i = 0;
  for(;i<len;i++){
...
int main(int argc, char** argv){
...
#endif
#ifdef TEST
  stype* arrTEST= createArrRand(randHolder);
#ifdef superVer
  for(int i=0;i<len;i++){
    verArr[i]=arrTEST[i];
  }
...
#endif
  gettimeofday(&t[0],NULL);
  qsort(arrTEST, len, sizeof(stype), intcomp);  
  gettimeofday(&t[1],NULL);
#ifdef ver
  verCorrect(arrTEST);
#endif
...
#endif
  free(arrTEST);
  doPrint(TEST,getDiff(t[0],t[1]));
#endif

#ifdef PPTEST
#ifdef raTablePtr
  int* tablePPTEST=calloc(tsize, sizeof(int));
  //  memset(tablePPTEST,-1,tsize*sizeof(int));
#else
  node** tablePPTEST=calloc(tsize, sizeof(node*));
#endif
  node* nodesPPTEST=calloc(len, sizeof(node));
  stype* arrPPTEST= createArrRand(randHolder);
  void* bigGuy=calloc(len+tsize+len, sizeof(int));
  memcpy(bigGuy, arrPPTEST, len*sizeof(stype));
  arrPPTEST=(stype*)bigGuy;
  memcpy(bigGuy+len*sizeof(stype), tablePPTEST, tsize*sizeof(stype));
  tablePPTEST=(int*)(bigGuy+len*sizeof(stype));
#ifdef superVer
  for(int i=0;i<len;i++){
    verArr[i]=arrPPTEST[i];
  }
  qsort(verArr, len, sizeof(stype), intcomp);  
#endif
  gettimeofday(&t[0],NULL);
  //  testSort4AfterGet(tablePPTEST, nodesPPTEST, arrPPTEST,qsort);
  testAddGet(tablePPTEST, arrPPTEST);
  gettimeofday(&t[1],NULL);
#ifdef ver
  verCorrect(arrPPTEST);
#endif
#ifdef superVer
  for(int i =0;i<len;i++){
    if(verArr[i]!=arrPPTEST[i]){
      printf("Error\n");
      exit(0);
    }
  }
  ...
  //  free(nodesPPTEST);
  //  free(arrPPTEST);
  doPrint(PPTEST,getDiff(t[0],t[1]));
#endif
#ifdef GRTEST
  stype* arrGRTEST= createArrRuns(randHolder, runSize, runOffset);
#ifdef superVer
  for(int i=0;i<len;i++){
    verArr[i]=arrGRTEST[i];
  }
  qsort(verArr, len, sizeof(stype), intcomp);  
#endif
  gettimeofday(&t[0],NULL);
  qsort(arrGRTEST, len, sizeof(stype), intcomp);  
  gettimeofday(&t[1],NULL);
#ifdef ver
  verCorrect(arrGRTEST);
#endif
#ifdef superVer
  for(int i =0;i<len;i++){
    if(verArr[i]!=arrGRTEST[i]){
      printf("Error\n");
      exit(0);
    }
  }
#endif
  free(arrGRTEST);
  doPrint(GRTEST,getDiff(t[0],t[1]));
#endif
#ifdef PPGRTEST
#ifdef raTablePtr
  int* tablePPGRTEST=malloc(tsize* sizeof(int));
  memset(tablePPGRTEST,-1,tsize*sizeof(int));
#else
  node** tablePPGRTEST=calloc(tsize, sizeof(node*));
#endif
  node* nodesPPGRTEST=calloc(len, sizeof(node));
  stype* arrPPGRTEST= createArrRuns(randHolder, runSize, runOffset);
#ifdef superVer
  for(int i=0;i<len;i++){
    verArr[i]=arrPPGRTEST[i];
  }
  qsort(verArr, len, sizeof(stype), intcomp);  
#endif
  gettimeofday(&t[0],NULL);
  testSort4AfterGet(tablePPGRTEST, nodesPPGRTEST, arrPPGRTEST, qsort);
  gettimeofday(&t[1],NULL);
#ifdef ver
  verCorrect(arrPPGRTEST);
#endif
#ifdef superVer
  for(int i =0;i<len;i++){
    if(verArr[i]!=arrPPGRTEST[i]){
      printf("Error\n");
      exit(0);
    }
  }
  ...
  free(nodesPPGRTEST);
  free(arrPPGRTEST);
  doPrint(PPGRTEST,getDiff(t[0],t[1]));
#endif
#ifdef CQS
